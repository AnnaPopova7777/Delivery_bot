...
async def process_cart(message: Message, state: FSMContext):
    ...
    if len(cart_data) == 0:
        ...
        if order_cost != 0:
            ...
            markup.add('+ Нет, хочу еще кое что добавить')
...
async def process_name(message: Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        markup_del = ReplyKeyboardMarkup(resize_keyboard=True, selective=True)
...
@dp.message_handler(IsUser(), text=back_message, state=CheckoutState.delivery_choice)
async def process_names_back(message: Message, state: FSMContext):
    await CheckoutState.check_cart.set()
    await checkout(message, state)
...
async def process_confirms_invalid(message: Message):
    await message.reply('Такого варианта не было.')


@dp.message_handler(IsUser(), text=back_message, state=CheckoutState.confirm)
async def process_confirm_adress(message: Message, state: FSMContext):
...
async def process_confirm_ord(message: Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        if data['delivery_c'] == 'Курьером':
            await message.answer(f'{answer}\nОбщая сумма заказа (без учета доставки курьером): {total_price_cur}₽.')
        ...
        await message.answer('Отлично! Менеджер свяжется с вами в ближайшие минуты.'
                             ...
                             '\n Что бы сделать новый заказ нажмите сюда /menu')

        await bot.send_message(ADMIN_ID, f'Ура! Новый заказ:'
                                           f'<b>Общая сумма заказа: {total_price}₽.</b>'
                                           f'\n{answer}'
                                           f'<b>\nИмя: </b>' + data['name'] +
                               f'<b>\nСпособ доставки: </b>' + data['delivery_c'] +
                               f'<b>\nНомер телефона: </b>' + data['number'] +
...
async def process_cart_del(message: Message, state: FSMContext):
    async with state.proxy() as data:
        cid = message.chat.id
        db.query('DELETE FROM cart WHERE cid=?', (cid,))
...
@dp.message_handler(IsUser(), text='+ Нет, хочу еще кое что добавить')
async def process_cart_add(message: Message, state: FSMContext):
    async with state.proxy() as data:
        await message.answer('Выберите раздел, чтобы вывести список товаров:',
                             reply_markup=categories_markup())
        markup_m = ReplyKeyboardMarkup(resize_keyboard=True, selective=True)
        markup_m.add(cart)

    await message.answer('Окей, выберите еще что-нибудь, '
                         'или перейдите в корзину', reply_markup=markup_m)
